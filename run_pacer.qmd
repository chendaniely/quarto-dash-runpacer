...
Here you can tweak some of the settings for the app.
...
```{python}
ui.input_text_area(
  id="SHY_REFERENCE_DISTANCES",
  label="Reference Distances\n(separate lines):",
  value="\n".join(REFERENCE_DISTANCES),
  height="75%",
  width="200px"
)
```
...
def shy_reference_distances():
  values = input.SHY_REFERENCE_DISTANCES().strip().split("\n")
  return ', '.join(values)

@reactive.Calc
def shy_set_reference_distances():
  return input.SHY_REFERENCE_DISTANCES().strip().split("\n")
